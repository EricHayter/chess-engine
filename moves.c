...
Piece* isOccupied(Board* board, uint8_t pos) {
    if ((board->bBishop >> pos) && 1) return (Piece) {BISHOP, BLACK};
    if ((board->wBishop >> pos) && 1) return (Piece) {BISHOP, WHITE};

    if ((board->bKing >> pos) && 1) return (Piece) {KING, BLACK};
    if ((board->wKing >> pos) && 1) return (Piece) {KING, WHITE};

    if ((board->bKnight >> pos) && 1) return (Piece) {KNIGHT, BLACK};
    if ((board->wKnight >> pos) && 1) return (Piece) {KNIGHT, WHITE};

    if ((board->bPawn >> pos) && 1) return (Piece) {PAWN, BLACK};
    if ((board->wPawn >> pos) && 1) return (Piece) {PAWN, WHITE};

    if ((board->bQueen >> pos) && 1) return (Piece) {QUEEN, BLACK};
    if ((board->wQueen >> pos) && 1) return (Piece) {QUEEN, WHITE};

    if ((board->bRook >> pos) && 1) return (Piece) {ROOK, BLACK};
    if ((board->wRook >> pos) && 1) return (Piece) {ROOK, WHITE};

   return EMPTY; // empty spot on board
}

ArrayList* bishopMoves(Board* board, unint8_t pos) {
    for (int i{1}; i < (8 - col) & i < (8 - col); i++) {
          p = {b[row][col + i]};
          if (p.color == piece.color)
            break;
